...
class Level_01(Level):
    ...
    def __init__(self, player, lvl=DEFAULT_LVL):
...
def rematch(players, level, spawns, r_list, active_sprite_list, FIGHT):
    #rematch setup
    FIGHT.play()
    i=0
    for p in players :
        p.level = level
        spawn = spawns[r_list[i]]
        p.rect.x = spawn[0]
        p.rect.y = spawn[1]
        p.lives = 3
        p.bullets = []
...
#NETWORK
    ...
    connection.connect(('192.168.0.5', 12345))
    acc = connection.recv(BUFFER)
    print acc
    my_p = int(re.findall('\d', acc)[0])-1
    connection.send('ok')

    start_info = connection.recv(BUFFER)
    start_info = map(int, start_info.split(','))



#SCREEN
    # Set the height and width of the screen
    #size = [SCREEN_WIDTH2, SCREEN_HEIGHT2]

    #Get display size of monitor
    infoObject = pygame.display.Info()
    size = (int(infoObject.current_w/2), int(infoObject.current_h/2))
    ...
    player1 = Player('Gwendal', RED, 0, sprite='./img/gwendal.png')
    player2 = Player(u'El Péhème', BLUE, 1, sprite='./img/elpeheme.png')
    player3 = Player('Huberr', GREEN, 1, sprite='./img/huberr.png')
    player4 = Player('Bastien', PINK, 2, sprite='./img/bastien.png')
    ...
    players[chosen[my_p]].set_kb(all_kb[0])
    ...
    level_list = []
    for l in all_levels :
            level_list.append( Level_01(players[0], lvl = l))
    ...
    while not done:
        ...
        #print "SENT",len(tosend)
        connection.send(tosend)

        all_data = connection.recv(BUFFER)
        #print all_data
        all_data = all_data.split(';')
        lv_info = map(int,all_data[0].split(','))
        ...
        bullet_hit([players[index] for index in alive_players], spawns[lv_info[1]])
        ...
        if reset == True:
            ...
            #Change level
            current_level_no = lv_info[0]
            current_level = level_list[current_level_no]
            spawns = all_spawns[current_level_no]
            rematch(players, current_level, spawns, lv_info[1:], active_sprite_list, PREPARE[randint(0,len(PREPARE)-1)])
